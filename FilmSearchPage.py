...
class FilmSearchPage(tk.Frame):
    ...
    def update(self):
        ...
        key = ApiKey.give_key()
        film_list = FilmApi(key)

        self.controller.values["result"] = film_list.get_intro(self.controller.values["query"].get())

        row = 1
        for film in self.controller.values["result"]:
            if film["image"] == None:
                tempImg = tk.Label(self.content, text="No Image", font=("Calibri italic", 25))
            else:
                image = self.urltoImg(film["image"], 200, 300)
                tempImg = tk.Label(self.content, image=image)
                tempImg.image = image
            tempImg.grid(row=row, column=0)
            temp = tk.Button(self.content, text=f'{film["title"]}', font=("Calibri italic", 25),
                             command=partial(self.film, row-1))
            temp.grid(row=row, column=1)
            row += 1

    def film(self, row):
        self.frame.destroy()
        self.controller.values["film"] = self.controller.values["result"][row]
        self.controller.show_frame("FilmPage")
